...
data_15m_qto1=read_complex_binary ("5m_qty_110mv_121_1.bin", 1000000,13001);
...
RSSI_5m_qt=mean(RSSI_15m_qt_f(1:500));
figure(5)
plot(angle_15m_qt_f)
...
distance_5m_qt=mean(diff_angel_15m_qt)/720*300; % Distance Estimation 0.0613
error_5m_qt=distance_5m_qt/5-1; %Estimation error is 0.0613

for j=1:120
    mean_mag_15m_qt(j)=20*log10(mean(abs(data_15m_qt_0((j-1)*500+1:j*500))));
end
x=10.^(mean_mag_15m_qt(1:101)/10).*cos(-mean_angel_15m_qt(1:101)/180*pi)+1i*10.^(mean_mag_15m_qt(1:101)/10).*sin(-mean_angel_15m_qt(1:101)/180*pi)
plot(abs(ifft(x.*x)))

figure(8)
plot(angle_15m_qt_f(1:500*13)+180,'k')
hold on
grid on
plot(angle_15m_nt_f(1:500*13)+180,'b')
xlabel('# of Samples')
ylabel('\phi_{RSP}(deg)')
set(gca,'XTick',[0:1000:6500])
legend('Tunneling Tag','Semi-passive Tag','Location','southeast')
matlab2tikz('phaseplot.tex');
